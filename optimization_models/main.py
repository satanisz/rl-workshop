...
from train import train_rl, train_sl
...
def evaluate_models(n_test_items=20, n_samples=50):
    ...
    train_sl(model_sl, n_epochs=20, batch_size=32, n_train_items=n_test_items)
    model_sl.eval()

    print("\nTraining Reinforcement Learning Model (RL)...")
    model_rl = KnapsackTransformer(nhead=4, num_layers=2)
    train_rl(model_rl, n_epochs=20, batch_size=32, n_train_items=n_test_items)
    model_rl.eval()
